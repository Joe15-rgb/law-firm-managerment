   ...
   static async index(req: Request, res: Response) {
      try {
         ...
         const [assignedLedgers, groups, groupLedgers] = await Promise.all([
            ...
            service.getGroupAssignedLedgersByCurrentLawyer(
         ...
         LawyerControllers.handleControllerError(
            res,
            error,
            'Erreur lors du rendu de la page avocat'
         );
      }
   }

   static readMyLegalCase = asyncMiddleware(async (req, res) => {
      try {
        const id = req.params.id
        const myLegaCase = await prisma.legalCase.findUnique({
          where: {id},
          include:{
            client: true,
            documents: true
         ...
         res.status(200).render('pages/lawyer/show', {myLegaCase})
      } catch (error) {
         LawyerControllers.handleControllerError(
            res,
            error,
            'Erreur lors du rendu de la page du dossier avocats'
         );
      }
   });

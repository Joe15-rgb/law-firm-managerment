...
import { middlewares, routes, plugins } from '@ioc/kernel';
...
if (cluster.isPrimary) {
  ...
  cluster.on('exit', (worker, code, signal) => {
    console.log(chalk.yellow(`Worker ${worker.process.pid} exited with code ${code} and signal ${signal}. Restarting...`));
    cluster.fork();
  });
